...
float *ReadImage(const std::string &filename,
                 int &width,
                 int &height,
                 const int &channel)
{
...
float *ReadImageLayer(const std::string &filename,
                      ...
                      const int &channel)
{
    ...
    float *out; // width * height * RGBA
    const char *err = nullptr;
    int ret = LoadEXRWithLayer(&out, &width, &height, filename.c_str(), layername.c_str(), &err);
    float *buffer = new float[width * height * channel];
