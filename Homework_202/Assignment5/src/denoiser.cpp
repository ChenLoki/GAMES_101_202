...
void Denoiser::Reprojection(const FrameInfo &frameInfo)
{
    ...
    // 上一帧中，世界坐标到屏幕坐标的变换矩阵
    Matrix4x4 pre_World_To_Screen = m_preFrameInfo.m_matrix[m_preFrameInfo.m_matrix.size() - 1];
...
#pragma omp parallel for
    for (int y = 0; y < height; y++)
    {
        for (int x = 0; x < width; x++)
        {
            ...
            // 如果当前帧的(x,y)像素点对应的物体为空
            if (id == -1)
            {
                continue;
            }

            /***** 查找当前像素(x,y)对应的前一帧的屏幕坐标 ******/
            // 当前帧的物体不会空，id有效
            // m_matrix矩阵按照物体编号，matrix : object->world
            // 取逆矩阵，就是 world->object，将当前帧的世界坐标，变换到物体的object坐标
            ...
            auto pre_local_position  = world_to_local(world_position,           Float3::EType::Point);// 转换到物体的object_pos
            auto pre_world_position  = pre_local_to_world(pre_local_position,   Float3::EType::Point);// 转换到前一帧的world_pos
            auto pre_screen_position = pre_World_To_Screen(pre_world_position,  Float3::EType::Point);// 转换到前一帧的屏幕坐标
        ...
        for (int x = 0; x < width; x++)
        {
            ...
            for (int m = x_start; m <= x_end; m++)
            {
                for (int n = y_start; n <= y_end; n++)
                {
                    ...
                    auto d_position = SqrDistance(center_postion, postion) / (2.0f * m_sigmaCoord * m_sigmaCoord);
...
Buffer2D<Float3> Denoiser::ProcessFrame(const FrameInfo &frameInfo)
{
    ...
    if (m_useTemportal)
    {
        Reprojection(frameInfo);// 找到前一帧的像素值
